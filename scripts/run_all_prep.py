...
# All counties that show up in counties.txt / single_policy_data.csv
COUNTIES = [
...
def main():
    ...
    for state, state_counties in sorted(state_to_counties.items()):
        ...
        script_path = os.path.join(project_root, 'scripts', 'data_prep.sh')
        
        # Run data_prep.sh
        result = subprocess.run(['bash', script_path], cwd=project_root, env=env)
        if result.returncode != 0:
            print(f"Error: data_prep.sh failed for state {state} (exit code: {result.returncode})")
        else:
            print(f"Successfully completed data preparation for state {state}")
